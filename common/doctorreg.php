...
$msg = "";
	if(isset($_POST['txt_name']))
	{
		...
		$sql = "insert into login values('$mail','$pwd','doctor',0)";
		setDatas($sql);
		$sql = "insert into doctorreg values('$name','$mail','$addr','$contact','$specialization','$experiance')";
		setDatas($sql);
		$msg = mysql_error();
		if(!isset($msg) || $msg=='')
		{
					...
					<div class="col-md-6 col-md-offset-1 contact-form">
						...
						<input class="" type="text" name="txt_name" required="required" placeholder="Name"  onkeypress="return verifyText(event)" /><br>
							  ...
							  <input type="password" placeholder="Confirm Password" name="txt_user_cpwd" id="txt_user_cpwd" onblur="confirmPwd('txt_user_pwd','txt_user_cpwd','err')" required="required"/>
		                       <input type="text" name="txt_specialization" required="required" placeholder="Specialization"  onkeypress="return verifyText(event)" /><br>
							    <input type="text" name="txt_experiance" required="required" placeholder="Experiance"  onkeypress="return verifyText(event)" /><br>
